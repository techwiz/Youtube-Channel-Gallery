	...
	private function ytchag_rss_markup( $instance ) {
		...
		if ( empty( $ytchag_user ) ) {
			...
			if ( $ytchag_feed == 'user' ) {
				...
				// NEW CODE TECHWIZ
				$ytchag_rss_url  = $youtube_feed_url . 'channel_id=' . $ytchag_user . '&max-results='. $ytchag_maxitems;
				$ytchag_link_url  = $youtube_url . '/channel/' . $ytchag_user;
			}
			if ( $ytchag_feed == 'favorites' ) {
				...
				// NEW CODE TECHWIZ
				$ytchag_rss_url  = $youtube_feed_url . '/channels/' . $ytchag_user . '/favorites';
        ...
        //OLD CODE TECHWIZ
				...
				//NEW CODE TECHWIZ
				$ytchag_rss_url = $youtube_feed_url . '/playlist_id=' . $ytchag_user . '&start-index=' . $startindex . '&max-results=' . $ytchag_maxitems;// . '&orderby=reversedPosition';
		...
		if ( $response_code != 200 ) {
			...
			if ( $ytchag_link ) {
				$content.= '<div class="ytcmore"><a href="' . $ytchag_link_url . '" class="ytcmore" ' . $ytchag_link_window .' ' . $ytchag_nofollow .' >' . $ytchag_link_tx . '</a></div>';
			}
			//--}
		}
